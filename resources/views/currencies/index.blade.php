...
@section('content')
    ...
    <div class=" px-4 ">
        ...
        <div >
            <a href="{{route('currencies.create')}}" class="btn btn-primary">Добавить</a>
        </div>
        ...
        <table class="table table-hover table-striped">
            ...
            @foreach($currencies as $currency)
                <tr id="demo" onclick="myFunction()">
                    <th scope="row">{{$currency->id_number}}</th>
                    <td >{{$currency->character_code}}</td>
                    <td>{{$currency->name}}</td>

                    <td>
                        <a href="{{route('currencies.edit', $currency->id)}}" class="fa fa-pencil">
                        ...
                        <form method="POST" action="{{route("currencies.delete", $currency->id)}}">
                            @csrf
                            @method("DELETE")
