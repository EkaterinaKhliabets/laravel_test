    ...
    <div class=" px-4 ">
        ...
        <table class="table table-hover table-striped">
            ...
            @foreach($bankAccounts as $bankAccount)
                <tr>
                    <th scope="row">{{$bankAccount->bank->name}}</th>
                    <td>{{$bankAccount->currency->name}}</td>
                    <td>{{$bankAccount->checking_account}}</td>

                    <td>
                        <a href="{{route('bankAccounts.edit', $bankAccount->id)}}" class="fa fa-pencil">
                        ...
                        <form method="POST" action="{{route("bankAccounts.delete", $bankAccount->id)}}">
                            @csrf
                            @method("DELETE")
